...
def card(study, index):
    ...
    return f'''<a class="study-card study-card--{e(study['slug'].split('-')[0])}" href="studies/{e(study['slug'])}.html" data-category="{e(category.lower())}" data-industry="{e(study['industry'].lower())}"><div class="card-cover" aria-hidden="true"><span class="cover-mark">{e(study['brand'])}</span><span class="cover-number">{index:02d}</span></div><div class="card-top"><span class="card-index">{label}</span><span class="card-brand">{e(study['brand'])}</span></div><div class="card-body"><p class="eyebrow">{e(study['industry'])} <span aria-hidden="true">/</span> {e(study['lens'])}</p><h3>{e(study['title'])}</h3><p>{e(study['dek'])}</p></div><div class="card-bottom"><span>Read the analysis</span><span aria-hidden="true">↗</span></div></a>'''
