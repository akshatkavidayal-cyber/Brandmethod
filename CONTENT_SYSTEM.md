...
Start from a particular observation or friction. State an argument, show what the brand actually did, support it with dated and linked primary/public evidence, distinguish inference from fact, and explain what evidence is missing. Use the article's natural structure; never force every piece into observed/interpreted/challenged/test. Write like Akshat: curious, opinionated, specific, sometimes irritated, never artificially certain. No fake metrics or invented campaign outcomes. Every published article must display the subject brand logo on its homepage card and article page. Source the mark from an official brand media page or a reputable free logo library, store it locally in `assets/`, and record attribution in `assets/LOGOS.md`. Logo sourcing, insertion and routine visual checking are part of publication and do not require separate approval.
...
5. Create a LinkedIn adaptation with one hook, the central argument, a clear link to the live article and an invitation to discuss. Create one supporting LinkedIn image using original typography/collage, an original diagram or appropriately licensed/public evidence. Instagram is outside the workflow unless Akshat explicitly requests it again.
6. Add the approved article to `content/studies.json` or `content/from-linkedin.json`, add the brand logo and supporting images to `assets/`, record the logo source in `assets/LOGOS.md`, and connect the article to the most relevant practice/work/research page plus 1–2 related essays. Add an article-specific link from an existing page where it reads naturally. `scripts/build.py` generates canonicals, metadata, structured data and sitemap.
...
Bundle user decisions. A release may require at most four approvals: topic/direction when genuinely needed, article/argument, visual, and final publication. Routine research, public-source verification, logo sourcing and insertion, formatting, validation, internal-link updates, GitHub commits and post-publication checks are pre-authorized once Akshat asks for the work. Do not request separate permission for each substep.
